      ...
      <div className="flex-1 min-h-0 overflow-x-auto">
        <div className="grid grid-cols-7 h-full min-w-7xl gap-0">
          {days.map((day, i) => (
            ...
            >
              {i === 0 ? (
                <div className="mx-2 text-sm font-medium text-center mb-2 bg-red-500/20 border border-red-500 text-red-500 rounded-sm px-2 py-1">
                {day.label}
                </div>
              ) : (
                <div className="text-sm font-medium text-center shrink-0 py-1 mb-2">
                  {day.label}
                </div>
              )}
                ...
                {day.assignments.length === 0 ? (
                  ...
                  day.assignments.map((assignment) => (
                    <Link
                      ...
                      className={`block text-xs px-2 py-1.5 rounded-sm border transition-opacity hover:opacity-90 shrink-0 ${
                    ...
                    >
                      ...
                      { assignment.completed ? <CheckCircle className="size-3 text-green-500" /> : <XCircle className="size-3 text-red-400  " />}
                      </div>
                      <div className="flex flex-row justify-between">
                      {assignment.due_date && (
